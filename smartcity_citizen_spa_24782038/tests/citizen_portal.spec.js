...
//   Opsi 1 (Live Server): 'http://127.0.0.1:5500/smartcity_citizen_spa_24782038/index.html'
 ...
 *
 * Menggunakan pola URL yang mengandung /api/, fungsi ini akan mencegat request ke domain apapun
...
test.describe('Modul 1: Otorisasi & Sesi (AUTH-04, AUTH-05, AUTH-06)', () => {
    ...
    test('AUTH-06: Kedua token kadaluarsa → localStorage dibersihkan, redirect ke #login', async ({ page }) => {
        ...
        // 5d. Form login harus terlihat (verifikasi visual)
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    test('UI-02: Live Search pada daftar laporan admin berfungsi', async ({ page }) => {
        ...
        const tableBody   = page.locator('#report-table-body');
        ...
        const responsePromise = page.waitForResponse(
            (response) => response.url().includes(`/ajax/live-search/?q=${searchKeyword}`) && response.status() === 200,
            { timeout: 15000 }
        );
        ...
        console.log(`[UI-02] Hasil pencarian "${searchKeyword}": ${responseData.reports?.length || 0} item`);
        ...
        if (responseData.reports && responseData.reports.length > 0) {
            ...
            expect(filteredRowCount).toBe(responseData.reports.length);
        }
    ...
    test('UI-03: Pagination Feed Kota — maks 10 kartu, kontrol pagination muncul', async ({ page }) => {
        ...
        // -------------------------------------------------------------------
        await page.goto(`${SPA_URL}#dashboard`);
        await page.waitForSelector('button:has-text("Tambah Laporan Baru")', { state: 'visible', timeout: 10000 });

        // -------------------------------------------------------------------
        // LANGKAH 4: Klik tab "Feed Kota (Publik)"
        // -------------------------------------------------------------------
        const tabFeedKota = page.locator('button:has-text("Feed Kota")');
        ...
        const listContainer = page.locator('#reportContainer');
        await expect(listContainer).toBeVisible();

        const reportCards = listContainer.locator('.col-lg-6');
    ...
    test('UI-04: Klik tombol Buat Laporan → modal #reportModal muncul', async ({ page }) => {
        ...
        const btnBukaModal = page.locator('button:has-text("Tambah Laporan Baru")');
        ...
        await expect(page.locator('button:has-text("Simpan Draft")')).toBeVisible();
        await expect(page.locator('button:has-text("Ajukan")')).toBeVisible();

        // Verifikasi judul modal
        const modalTitle = page.locator('#reportModal .modal-title');
        await expect(modalTitle).toContainText('Form Laporan');
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        await page.locator('button:has-text("Tambah Laporan Baru")').click();
        ...
        await page.locator('#category').fill('Infrastruktur');
        ...
        const summaryStats = page.locator('#summaryContainer');
