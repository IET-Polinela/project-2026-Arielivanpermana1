...
class PrivacyAndDataHidingTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.laporan_publik_admin = Report.objects.create(
            title='Info Perbaikan Jalan Kota',
            category='Infrastruktur',
            description='Petugas sedang meninjau jalan rusak.',
            location='Pusat Kota',
            status='REPORTED',
            reporter=self.admin,
        )
    ...
    def test_PRIV_02_laporan_saya_menampilkan_nama_asli(self):
        ...
        # Verifikasi: reporter untuk laporan sendiri menampilkan username asli.
        for laporan in results:
            self.assertEqual(
                laporan['reporter'],
                'warga_a',
