...
class SerializerAndModelCoverageTests(APITestCase):
    ...
    def test_report_serializer_no_request_context(self):
        ...
        self.assertEqual(serializer.data['reporter'], 'warga_str_test')
...
class MainAppMonolithicViewsCoverageTests(TestCase):
    ...
    def setUp(self):
        ...
        self.draft_report = Report.objects.create(
            title='Draf Monolitik Uji',
            category='Infrastruktur',
            description='Draf milik citizen.',
            location='Bandung',
            status='DRAFT',
            reporter=self.citizen
        )
    ...
    def test_report_detail_api_valid(self):
        from main_app.views import report_detail_json
        from django.test import RequestFactory
        factory = RequestFactory()
        request = factory.get('/dummy-url/')
        request.user = self.admin
        response = report_detail_json(request, self.report.id)
    ...
    def test_report_detail_api_invalid(self):
        ...
        with self.assertRaises(Http404):
            report_detail_json(request, 99999)

    def test_report_search_unauthenticated(self):
        response = self.client.get(reverse('live_search_reports') + '?q=Lampu')
        self.assertIn(response.status_code, [200, 302, 403])

    def test_report_search_citizen(self):
        self.client.login(username='citizen_mono', password='Password123!')
        response = self.client.get(reverse('live_search_reports') + '?q=Monolitik')
        self.assertEqual(response.status_code, 200)
    ...
    def test_home_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'main_app/landing.html')
    ...
    def test_report_detail_view_unauthenticated(self):
        response = self.client.get(reverse('detail_report', kwargs={'pk': self.report.id}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_report_update_view_unauthenticated(self):
        response = self.client.get(reverse('edit_report', kwargs={'pk': self.draft_report.id}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_report_update_view_admin_post_valid(self):
        ...
        response = self.client.post(reverse('edit_report', kwargs={'pk': self.draft_report.id}), payload)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('report_list'))
        self.draft_report.refresh_from_db()
        self.assertEqual(self.draft_report.title, 'Laporan Terupdate')

    def test_report_delete_view_unauthenticated(self):
        response = self.client.get(reverse('delete_report', kwargs={'pk': self.draft_report.id}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_report_delete_view_admin_post(self):
        self.client.login(username='citizen_mono', password='Password123!')
        response = self.client.post(reverse('delete_report', kwargs={'pk': self.draft_report.id}))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('report_list'))
        self.assertFalse(Report.objects.filter(id=self.draft_report.id).exists())
    ...
    def test_report_delete_view_direct_delete_method(self):
        ...
        factory = RequestFactory()
        request = factory.post(reverse('delete_report', kwargs={'pk': self.draft_report.id}))
        ...
        view.setup(request, pk=self.draft_report.id)
